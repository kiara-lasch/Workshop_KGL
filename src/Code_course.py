...
# Datasets
folder = Path("C:\\Users\\2899043\\PhD\\Code_course")

# Import combined_data_ssp126, ssp245, ssp585
Combined_data_ssp126 = pd.read_csv(folder / "Datasets/deltas_30_ssp126_decr.csv")
Combined_data_ssp245 = pd.read_csv(folder / "Datasets/deltas_30_ssp245_decr.csv")
Combined_data_ssp585 = pd.read_csv(folder / "Datasets/deltas_30_ssp585_decr.csv")
...
def protect_open(ss, swh, SLR, prop_width, riv_levee, coastline, VLM, riv_wid, riv_len):
    ...
    V_levee_r = V_trap_r + (RSLR * b2_r * L_r) * 2          # Volume (incl subsidence) --> * 2 for both sides of the river 
...
# ACCOMMODATE ---------------------------------------------------------------------------------------------
volume_to_fill_columns = ['whole_urban_inundated_volume', 'total_inundation_volume', 'urban_inundated_volume']
...
for dataset, subset_results in datasets:
    for index, row in dataset.iterrows():
        ...
        for result in delta_results:
            result.update({
                'levee_req_pc': levee_req_pc,
                'pump_cap_pc': pump_cap_pc
            })
        ...
        for result in delta_results:
            result.update({
                'levee_req_po': levee_req_po,
                'river_width_po': river_width_po
            })

        # ACCOMMODATE calculation 
        for volume_to_fill_col in volume_to_fill_columns:
            volume_to_fill = row[volume_to_fill_col]
            number_years_acc = accommodate(acc_raise_0p5, acc_raise_1, acc_raise_10, sed_dis, volume_to_fill)
            for result in delta_results:
                result.update({
                    'acc_raise_0p5': acc_raise_0p5,
                    'acc_raise_1': acc_raise_1,
...
for dataset in [Equation_results_ssp126, Equation_results_ssp245, Equation_results_ssp585]:
    # Advance
    ...
    dataset['adv_12000'] = dataset['pump_cap_adv'].apply(lambda x: 1 if x < 12000 else 0)
    
    dataset['adv_number_years_50'] = dataset['number_years_adv'].apply(lambda x:1 if x < 50 else 0)
    dataset['adv_number_years_100'] = dataset['number_years_adv'].apply(lambda x:1 if x < 100 else 0)
    dataset['adv_number_years_25'] = dataset['number_years_adv'].apply(lambda x:1 if x < 25 else 0)

    # Advance - Combined indicator
    dataset['adv_offshore_depth_30'] = dataset['depth_10km_offshore_inclSLR'].apply(lambda x: 1 if x < 30 else 0)
    dataset['adv_offshore_depth_3'] = dataset['depth_10km_offshore_inclSLR'].apply(lambda x: 1 if x < 3 else 0)
    dataset['adv_offshore_depth_60'] = dataset['depth_10km_offshore_inclSLR'].apply(lambda x: 1 if x < 60 else 0)
    ...
    dataset['po_4p5'] = dataset['river_width_po'].apply(lambda x: 1 if x < 4500 else 0) # 4500 m = 4.5km
    ...
    # Retreat 
    dataset['ret_urb_ni_area'] = dataset['urban_non_inundated_area_land_availability'].apply(lambda x: 1 if x > 1 else 0)
    dataset['ret_ni_area'] = dataset['non_inundated_area_land_availability'].apply(lambda x: 1 if x > 1 else 0)
...
columns_to_extract = [
    'BasinID2', 
    'adv_1200', 'adv_600', 'adv_12000', 
    'adv_number_years_50', 'adv_number_years_100', 'adv_number_years_25',
    'adv_offshore_depth_30', 'adv_offshore_depth_3', 'adv_offshore_depth_60',
    'pc_1200',    'pc_600', 'pc_12000', 
    'po_9', 'po_4p5','po_90',
    'acc_raise_1', 'acc_raise_0p5',  'acc_raise_10',
    'ret_ni_area', 'ret_urb_ni_area', 'ret_out_delta'
]

# Create the threshold analysis datasets for each SSP scenario
threshold_analysis_ssp126 = Equation_results_ssp126[columns_to_extract].drop_duplicates(subset=['BasinID2'])
threshold_analysis_ssp245 = Equation_results_ssp245[columns_to_extract].drop_duplicates(subset=['BasinID2'])
threshold_analysis_ssp585 = Equation_results_ssp585[columns_to_extract].drop_duplicates(subset=['BasinID2'])
...
print("SSP126 - Advance Known Max:", threshold_analysis_ssp126['adv_CurrentKnown'].sum())
